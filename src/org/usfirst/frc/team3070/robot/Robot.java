...
public class Robot extends IterativeRobot implements Pronstants {
	...
	public void autonomousInit() {
		...
		if (DriverStation.getInstance().getGameSpecificMessage().length() > 0) {
			autonomous.gameData = DriverStation.getInstance().getGameSpecificMessage(); // Gets data from
																						// field/dashboard
	...
	public void testPeriodic() {
		...
		SmartDashboard.putString("SwitchPos", autonomous.switchPos);
		SmartDashboard.putString("StartPos", autonomous.startPos);
		SmartDashboard.putNumber("Gyro Valuve", prontoGyro.getOffsetHeading());
		SmartDashboard.putNumber("SpeedL", drive.talLM.getSelectedSensorVelocity(0));
		SmartDashboard.putNumber("SpeedR", drive.talRM.getSelectedSensorVelocity(0));
		// grabber goes in and out with button 2 & 3 on the left joystick
		grabber.teleop(joyL.getRawButton(3), joyL.getRawButton(2));
